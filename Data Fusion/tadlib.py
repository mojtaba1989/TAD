...
def img2radar_map(data, h, theta, phi, f, sensor_size):
    """
    ...
    output = D + Rt.dot(C.dot(Rp.dot(p_0)))
    return output


def reversePinHole(data, displacement, orientation, yaw, sensorSize, focal=K):
    """
    :param data: N by 2 or 4 (if data is anchor) normalized [v,u]
    :param displacement: 3 by 1 position vector of camera
    :param orientation: tilt angle in degree
    :param K: [fx 0 cx;0 fy cy;0 0 1] in pixel
    :param sensorSize: [rows, cols]
    :return: position vector correspond to each pixel
    """

    #anchor to midle point
    if data.shape[1] == 4:
        data[:, 0] = np.mean(data[:, [0, 2]], axis=1).T
        data[:, 1] = np.mean(data[:, [1, 3]], axis=1).T
        data = data[:, 0:-2].reshape((-1, 2))

    # [v,u]_normal->[u;v]_pixel
    pixel = np.flip(data, axis=1).T

    pixel = np.vstack((pixel, np.ones((1, data.shape[0]))))
    pixel[0, ] = pixel[0, ]*sensorSize[1]
    pixel[1, ] = pixel[1, ]*sensorSize[0]

    Kinv = np.linalg.inv(focal*[-1, -1, 1])

    Xc = Kinv.dot(pixel)

    c = np.cos(np.radians(orientation))
    s = np.sin(np.radians(orientation))
    Rt = np.array([[1, 0, 0], [0, c, s], [0, -s, c]])

    c = np.cos(np.radians(yaw))
    s = np.sin(np.radians(yaw))
    Rp = np.array([[c, 0, -s], [0, 1, 0], [s, 0, c]])

    p = np.zeros([3, data.shape[0]])
    p[2, ] = displacement[2]/(s-c*Xc[1, ])
    p[1, ] = p[2, ]*Xc[1, ]
    p[0, ] = p[2, ]*Xc[0, ]
    ...
    output = np.array(displacement).reshape(3, 1) + Rt.dot(C.dot(Rp.dot(p)))
    return output
...
def mid_detection(boxes):
    ...
    if boxes.shape[0] == 2:
        v = np.mean(boxes[:, [0, 2]], axis=1).reshape(-1, 1)
        u = np.mean(boxes[:, [1, 3]], axis=1).reshape(-1, 1)
...
def Cam_Radar(p_r, val_r, p_old, val_old, p_c, w, ll=np.nan, method='kalman'):
    ...
    if method == 'kalman':
        ...
        if not np.any(np.isnan(p_old)):
            p_old = np.array(p_old).reshape(2, 1)
            D[0, ] = dist(p_r, p_old)
            D[0, ] = D[0, ]/np.max(D[0, ])* w[0]

        if not np.any(np.isnan(val_old)):
            val_old = np.array(val_old).reshape(1, 1)
            D[1, ] = np.abs(val_r - val_old)
            D[1, ] = D[1, ] / np.max(D[1, ]) * w[1]

        if not np.any(np.isnan(p_c)):
            p_c = np.array(p_c).reshape(2, 1)
            if ~np.max(np.isnan(ll)):
                D[2, ] = distline(p_r, ll)
            else:
                D[2, ] = dist(p_c, p_r)
            D[2, ] = D[2, ] / np.max(D[2, ]) * w[2]
...
def update_measure(R, l_corner, w_trailer, var, var_t):
    ...
    omega = np.arcsin(w_trailer / l_corner / 2)
    missed_flag = False
    if np.isnan(R[0, 0]) or np.isnan(R[0, 1]):
        missed_flag = True
        if np.isnan(R[0, 0]):
    ...
    if missed_flag:
        if missed_marker_light == 'p':
            phi = np.degrees(np.arctan(R[0, 1] / R[1, 1]) - omega)
        elif missed_marker_light == 'd':
            phi = np.degrees(np.arctan(R[0, 0] / R[1, 0]) + omega)
    else:
        p = R[:, 0]
        d = R[:, 1]
        ...
        TF = np.array([[c, -s], [s, c]]).reshape(2, 2)
        d_rotated = TF.dot(d)
        m = np.mean([p_rotated, d_rotated], axis=0)

        phi = np.degrees(np.arctan(m[0] / m[1]))

    return phi, l_corner, w_trailer
...
def centerCalc(x, y, initial_center=(0, 0)):
    ...
    center, ier = optimize.leastsq(f, initial_center, Dfun=Df_2b, col_deriv=True)
...
def houghCenter(image, boxes):
    imageSize = image.shape[0:2]
    for box in boxes:
        box = (box * [imageSize[0], imageSize[1], imageSize[0], imageSize[1]]).astype(int)
        box += [-20, -20, 20, 20]
        imageTemp = cv2.cvtColor(image[box[0]:box[2], box[1]:box[3], :], cv2.COLOR_BGR2GRAY)
        circles = cv2.HoughCircles(imageTemp, cv2.HOUGH_GRADIENT, 1.2, 20,
                                   param1=50, param2=30, minRadius=7, maxRadius=14)
        if np.all(circles == None):
            circles = [[(box[3]-box[1])/2, (box[2]-box[0])/2, 10]]
        circles = np.array(circles).reshape(-1, 3)
        for circle in circles:
            try:
                centers = np.vstack([centers, [box[0] + circle[1],
                                               box[1] + circle[0]]])
            except:
                centers = [box[0] + circle[1], box[1] + circle[0]]
    try:
        centers = (np.array(centers) / [imageSize[0], imageSize[1]]).reshape(-1, 2)
    except:
