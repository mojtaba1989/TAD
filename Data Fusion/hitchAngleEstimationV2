...
if __name__ == '__main__':
    ...
    datatNum = len(dummy)
    if HOUGH:
        pointSelection = "-houghCircle"
    else:
        pointSelection = "-median"

    if trackEnable:
        PATH_TO_RESULTS = os.path.join(CWD_PATH, FILE_NAME, FILE_NAME + '-' + MODEL_NAME[16:] + pointSelection
                                       + '-Tracking' + '.csv')
    else:
        PATH_TO_RESULTS = os.path.join(CWD_PATH, FILE_NAME, FILE_NAME + '-' + MODEL_NAME[16:] + pointSelection
                                       + '.csv')
    ...
    for INDEX in range(datatNum):
        index, time_stamp, angle, x_p, y_p, range_p, peakVal_p, x_d, y_d, range_d, peakVal_d, p_r_set, d_r_set = pdf.readCSV(
            PATH_TO_CSV, INDEX)
        Results.loc[INDEX, "Vernier"] = np.array(angle)

        if RADAR:
            range_p   = np.array(range_p)
            range_d   = np.array(range_d)
            peakVal_p = np.array(peakVal_p)
            peakVal_d = np.array(peakVal_d)
            p_r_set   = np.array(p_r_set)
            d_r_set   = np.array(d_r_set)
            T_p = np.logical_and(range_p > 1, range_p < 2)
            if len(range_p[T_p]) == 1:
                p_r = p_r_set[:, T_p].reshape(2, -1)
                p_r = pdf.tm_f(p_r, .56, .85, .05, 22, 6, 'p')[0:-1, ]
            elif len(range_p[T_p]) > 1:
                p_r = p_r_set[:, T_p][:, peakVal_p[T_p].argmax()].reshape(2, -1)
                p_r = pdf.tm_f(p_r, .56, .85, .05, 22, 6, 'p')[0:-1, ]
            else:
                p_r = np.array([[np.nan], [np.nan]])
            T_d = np.logical_and(range_d > 1, range_d < 2)
            if len(range_d[T_d]) == 1:
                d_r = d_r_set[:, T_d].reshape(2, -1)
                d_r = pdf.tm_f(d_r, .56, .85, .05, 20, 6, 'd')[0:-1, ]
            elif len(range_d[T_d]) > 1:
                d_r = d_r_set[:, T_d][:, peakVal_d[T_d].argmax()].reshape(2, -1)
                d_r = pdf.tm_f(p_r, .56, .85, .05, 20, 6, 'd')[0:-1, ]
            else:
                d_r = np.array([[np.nan], [np.nan]])
        ...
        cam_det = pdf.reversePinHole(mid, [0, 0, .7], -6, 0, image.shape[:-1])
        ...
        if not estimateCenter:
            ...
            if RADAR:
                R_r = np.concatenate([p_r, d_r], axis=1) - center
                radar_phi, radar_l_corner, radar_w_trailer = pdf.update_measure(
                    R_r, radar_l_corner, radar_w_trailer, r_var, var_t)
                r_var, rdata = pdf.res_var(rdata, radar_l_corner, n=10)
            ...
            if RADAR:
                XY.loc[INDEX, "Xr_p"] = p_r[0]
                XY.loc[INDEX, "Xr_d"] = d_r[0]
                XY.loc[INDEX, "Yr_p"] = p_r[1]
                XY.loc[INDEX, "Yr_d"] = d_r[1]
    ...
    if estimateCenter:
        ...
        x = x[~np.isnan(x)]
        y = y[~np.isnan(y)]
        center, cam_l_corner, residual = pdf.centerCalc(x, y, initial_center=(0, 0))
        ...
        for INDEX in range(datatNum):
            ...
            if RADAR:
                p_r = np.empty((2, 1))
                d_r = np.empty((2, 1))
                p_r[0] = XY.loc[INDEX, "Xr_p"]
                d_r[0] = XY.loc[INDEX, "Xr_d"]
                p_r[1] = XY.loc[INDEX, "Yr_p"]
                d_r[1] = XY.loc[INDEX, "Yr_d"]
                R_r = np.concatenate([p_r, d_r], axis=1) - center
                radar_phi, radar_l_corner, radar_w_trailer = pdf.update_measure(
                    R_r, radar_l_corner, radar_w_trailer, r_var, var_t)
                r_var, rdata = pdf.res_var(rdata, radar_l_corner, n=10)
