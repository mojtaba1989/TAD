...
K = np.array([[585.8322184022027, 0.0, 620.377742120009], [0.0, 586.8191299992371, 480.06144414295363],
              [0.0, 0.0, 1.0]])
# K = np.array([[585.8322184022027, 0.0, 620.377742120009], [0.0, 506.8191299992371, 480.06144414295363],
#               [0.0, 0.0, 1.0]])
...
def update_measure(R, l_corner, w_trailer, l_p, l_d, var, var_t, missed_flag, missed_marker_light):#, origin):

    if var > var_t:
        l_corner_new = np.nanmean(np.sqrt(np.nansum(R ** 2, axis=0)))
        l_p_new = np.sqrt(np.nansum(R[:, 0] ** 2))
        l_d_new = np.sqrt(np.nansum(R[:, 1] ** 2))
        ...
        if not np.isnan(l_corner) and not np.isnan(w_trailer):
            if not np.isnan(l_corner_new) and not np.isnan(w_trailer_new):
                ...
                l_p = (var * l_p_new + l_p) / (var + 1)
                l_d = (var * l_d_new + l_d) / (var + 1)
    ...
    omega = np.arcsin(w_trailer / l_corner/1.4 / 2)*2
    ...
    return phi, l_corner, w_trailer, l_p, l_d#, origin
