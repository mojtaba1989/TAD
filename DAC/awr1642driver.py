...
class awr1642:
    ...
    def optimize(self, range_interval, eval_range):
        ...
        for i in range(10):
            ...
            time.sleep(.01)
            n = 90
            radar = np.zeros((n, n, 10))
            for itr in range(10):
                self.update()
                lim = 0
                while not self.dataOK and lim < 10:
                    self.update()
                    lim += 1
                if self.dataOK:
                    x, y, radar = pdf.heat_map(radar, self.detObj["x"], self.detObj["y"], self.detObj["peakVal"],
...
if __name__ == '__main__':
    driver = awr1642("profile.cfg", "R00410271", "R00410274")
    driver.sensorSetup()
    driver.setDetectionThreashold(1000)
    # driver.run()
